...
public class AggressiveAngel : MonoBehaviour
{
    [Header("Components")]
    ...
    //Out of 100
    public float teleChance = 1;
    private bool ableTele;
    ...
    void Start()
    {
        ...
        //Adds all the teleporters
        teleporters = new Transform[GameObject.Find("Teleporters").gameObject.transform.childCount];
        for (int i = 0; i < teleporters.Length; i++)
        {
            teleporters[i] = GameObject.Find("Teleporters").gameObject.transform.GetChild(i);
        }
    }
    ...
    void Update()
    {
        ...
        if (!flashlight.GetComponent<LightFollow>().isOn || !isBeingViewed())
        {
            ...
            //Teleportation
            if(canTele && (Random.Range(1, 101) <= teleChance) && ableTele)
            {
                while(ableTele == true)
                {
                    int teleSelect = Random.Range(0, teleporters.Length);
                    if (Vector3.Distance(teleporters[teleSelect].transform.position, player.gameObject.transform.position)! > 20)
                    {
                        transform.position = teleporters[teleSelect].transform.position;
                        ableTele = false;
                    }
                }
            }
